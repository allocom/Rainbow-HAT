...
"""
Store the rotation of RainbowHat, defaults to
...
def set_layout(pixel_map = AUTO):
    """Set the layout to Rainbow HAT or Rainbow pHAT

    Note: auto detection relies upon the HAT EEPROM. Your Rainbow HAT
...
def brightness(b=0.2):
    ...
    0.2 is highly recommended, RainbowHat can get painfully bright!
    ...
    RainbowHAT draws too much current above 50%."""
    brightness = int(b*128.0)
    if brightness < 30:
        print("Warning: Low brightness chosen, your RainbowHAT might not light up!")
...
def off():
    """Clear the buffer and immediately update RainbowHat
...
def show():
    """Update RainbowHat with the contents of the display buffer"""
    ws2812.show()
